...
test_that("occurrences works on a different server", {
    ...
    server_config$base_url_biocache <- "https://records-ws.nbnatlas.org/"
    ## or "http://datos.gbif.es/biocache-service/"
    ## previously used http://recherche-ws.gbif.fr/ but was down in May 2018
    options(ALA4R_server_config = server_config)
    ## we make really sure that the settings get put back to ALA ones here
    ...
    dotest <- function() {
        ...
        try({
            ## previous search was for Apus apus, which has a lot of
            ## occurrences and was slow
            ## now searching Onygena apus, which should only have one
            ## occurrence on NBN, so hopefully quicker
            occx <- occurrences(taxon = "Onygena apus",
