...
thischeck <- function() {
    test_that("specieslist returns empty data.frame for no results", {
        skip_on_cran()
        expect_warning(x <- specieslist("dfjknnaklhkjf",
    ...
    test_that("specieslist checks fq fields", {
        ...
        expect_warning(x <- specieslist(wkt="POLYGON((145 -37,150 -37,150 
                                        -30,145 -30,145 -37))",
                                        fq="rk_genus:Heleioporus"))
        expect_equal(nrow(x),0)
        x <- specieslist(wkt="POLYGON((145 -37,150 -37,150 -30,145 -30,
                         145 -37))",fq="genus:Heleioporus")
        expect_gt(nrow(x),0)
    })
