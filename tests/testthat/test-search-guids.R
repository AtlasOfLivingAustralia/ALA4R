...
thischeck=function() {
    test_that("search_guids returns occurrence counts when asked", {
        skip_on_cran()
        expect_false(is.na(search_guids("urn:lsid:biodiversity.org.au:afd.name:292120",occurrence_count=TRUE)$occurrenceCount))        
        expect_equal(is.na(search_guids(c("urn:lsid:biodiversity.org.au:afd.name:292120","isdulfsadh"),occurrence_count=TRUE)$occurrenceCount),c(FALSE,TRUE))
