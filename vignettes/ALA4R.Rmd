...
record identifier. We first do an occurrence search using facets to filter the records we want, then call an image function to query the image service.

The image download is rate limited to 1 image/second to minimise server load.
...
tryCatch({
  ...
  # pass the occurrence id to the occurrence_images to 
  # get the image metadata and optionally download to file
