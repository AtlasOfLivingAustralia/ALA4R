...
temp <- opts_knit$get("rmarkdown.pandoc.to")
if (!is.null(temp) && !is.na(temp) && grepl("html",temp)) opts_chunk$set(out.extra='style="display:block; margin: auto"', fig.align="center")
```
...
```{r}
to_install <- c("jpeg", "phytools", "ape", "vegan", "mgcv", "geosphere", "maps", "mapdata", "maptools")
...
head(x[order(x$occurrenceCount, decreasing=TRUE), c("speciesName", "commonName", "occurrenceCount")], 20)
```
...
```{r}
x <- occurrences(taxon="taxon_name:\"Amblyornis newtonianus\"", download_reason_id=10)
summary(x)
```
...
thiscol <- c("#1f77b4", "#ff7f0e", "#2ca02c", "#d62728", "#9467bd", "#8c564b", "#e377c2", "#7f7f7f", "#bcbd22", "#17becf")
