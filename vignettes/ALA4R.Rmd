...
# Identify records the occurrence records we are interested in and which have images associated with them.
penguin_occs <- occurrences(taxon="taxon_name:\"Eudyptes chrysocome\"",
                            fq="multimedia:Image", email = "test@ala-test.org", download_reason_id = "testing")

# Select only human observation records
image_ids <- penguin_occs$data %>%
  filter(basisOfRecord == "HumanObservation") %>%
  select(id) %>% pull(id)

# Download images where the image is available under a creative commons 4.0 licence
occurrence_images(image_ids, fq = "recognisedLicence:CC BY-NC 4.0", download = TRUE)
```
