...
shape <- read_sf(file.path(ala_config()$cache_directory, "CONSERVATION_NpwsaReserves.shp"))
...
```{r}
lonlat <- st_coordinates(shape) ## extract the polygon coordinates
...
dr_assert <- dr %>% select(name, all_of(assert_match)) %>%
  pivot_longer(-name, names_to = "assertion", values_to = "count") %>%
