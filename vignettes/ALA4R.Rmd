...
tryCatch(library(phytools), error=function(e) { install.packages("phytools",repos="http://cran.us.r-project.org") }) ## needed to ensure phytools is installed on rforge
tryCatch(library(mapdata), error=function(e) { install.packages("mapdata",repos="http://cran.us.r-project.org") }) ## ditto
```
...
You may need to install these packages first using `install.packages(c("ape","phytools"))`:
...
```{r}
imfiles=sapply(s,function(z){ ifelse(any(grepl("(\\.jpg)",z$images$smallImageUrl,ignore.case=TRUE)),
ALA4R:::cached_get(z$images$smallImageUrl[grepl("(\\.jpg)",z$images$smallImageUrl,ignore.case=TRUE)][1],
type="binary_filename"),"") })
```

And finally, plot the tree:
```{r results='hide', dev.args=list(colortype="pseudo.cube")}
