...
Then install the ALA4R package itself from the precompiled binaries provided on RForge:
```{r eval=FALSE}
install.packages("ALA4R",repos="http://rforge.net/")
```

Windows users may need to install from source, if binary packages are not available:
```{r eval=FALSE}
