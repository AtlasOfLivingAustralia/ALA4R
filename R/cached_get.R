...
#
# Convenience wrapper for web GET operations. Caching, setting the user-agent
...
# "json", or "filename" (this caches the content directly to a file and
# returns the filename without attempting to read it in)
# @param caching string: caching behaviour, by default from
# ala_config()$caching
# @param on_redirect, on_server_error, on_client_error function: passed to
# check_status_code()
# @return for type == "text" the content is returned as text. For
# type == "json", the content is parsed using jsonlite::fromJSON.
...
# the cache or from the url, and stored in the cache if appropriate.
# The user-agent string is set according to ala_config()$user_agent.
# The returned response (if not from cached file) is also passed to
# check_status_code().
# @references \url{https://api.ala.org.au/}
# @examples
#
# out <- cached_get(url = "https://biocache-ws.ala.org.au/ws/index/fields",
# type = "json")
#

cached_get <- function(url, type = "text", caching = ala_config()$caching,
                       verbose = ala_config()$verbose, on_redirect = NULL,
                       on_client_error = NULL, on_server_error = NULL,
                       encoding = ala_config()$text_encoding) {
    assert_that(is.notempty.string(url))
    assert_that(is.string(type))
    type <- match.arg(tolower(type),
                      c("text", "json", "filename", "binary_filename"))
    ...
    if (identical(caching, "off") &&
        !(type %in% c("filename", "binary_filename"))) {
    ...
    ## use caching
    thisfile <- download_to_file(url, outfile = this_outfile,
                                 binary_file = identical(type,
                                                         "binary_filename"),
                                 verbose = verbose, on_redirect = on_redirect,
    ...
    if (!file.exists(thisfile)) {
        ...
        if (type %in% c("json", "text")) {
            if (!(file.info(thisfile)$size > 0)) {
                ...
                ## for json, previously we read as string first, then passed
                ## this to fromJSON. This was compatible with either RJSON's or
                ## rjsonlite's version of fromJSON (only RJSON's version can
                ## take a filename directly). However, it introduced issues
                ## with readLines and handling of encoding. Now we pass the
                ## file directly to jsonlite::fromJSON
                if (type == "json") {
                    ## convert directly from file - this also allows jsonlite
