...
#'
#' @export
fieldguide <- function(guids, title = "Field guide",
    ...
    if (caching %in% c("on") && file.exists(cache_file)) {
        ...
        try({
            if (verbose) {
                message(sprintf("trying cached file %s for %s",
                                cache_file, this_url))
            }
    ...
    if (is.null(fileid)) {
        x <- POST(url = this_url, body = temp,
                  user_agent(ala_config()$user_agent), encode = "json")
        ## x response should have header with Fileid:
        ## 30082011/fieldguide1314682018564.pdf
        temp <- headers(x)
        if (is.null(temp$fileid)) {
            warning("The field guide generator failed. ",
