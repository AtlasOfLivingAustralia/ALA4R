                             ...
                             cex=0.75, ...) 
{
    ...
    if (missing("pch")) {
        ...
        if (length(pch)>1) { pch <- pch[1];
        warning("only using first element of supplied pch vector") }
        if (nchar(pch)>1) { pch <- substr(pch, 1, 1);
...
###plot function to be used
    tplot <- function(xx, Main, coi, pch) {
        ...
        if (is.null(coi)) {
            legend("bottomleft", legend="assumed good", pch=pch, col="black",
                   bty="n", cex=cex)
        } else {
            ...
            for (ii in 1:length(coi)) {
              #define the points that have the issue
                roi <- which(as.logical(xx[, coi[ii]])==TRUE)
                if (length(roi) > 0) {
                    points(xx$longitude[roi], xx$latitude[roi], pch=pch,
                           col=legend.cols[ii])
                    c2use <- c(c2use, ii)
                }				
            }
    ...
    tryCatch({
        if (grouped) {
            ...
            for (spp in unique(x$data[, taxon_level])) {
                ...
                if (spp!="") { 
                    tplot(x$data[which(x$data[, taxon_level]==spp), ], Main=spp,
                          coi=qa, pch)
                } else {
                    tplot(x$data[which(x$data[, taxon_level]==spp), ],
                          Main="unmatched species name", coi=qa, pch)
                }
            }
        }
