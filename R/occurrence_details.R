...
#'
#' @param uuid string: one or more record ids, as returned by
...
occurrence_details <- function(uuid, verbose=ala_config()$verbose) {
    ...
    this_url <- vapply(uuid[non_empty], function(z) {
        build_url_from_parts(getOption("ALA4R_server_config")$
                                 base_url_biocache, paste0("occurrence/", z))
    }, FUN.VALUE = "", USE.NAMES = FALSE)
    out_non_empty <- lapply(this_url, function(z) {
        cached_get(z, type = "json", verbose = verbose)
    })
    out <- vector("list", length(uuid))
    non_empty_which <- which(non_empty)
    for (k in seq_len(length(uuid))) {
        if (non_empty[k]) {
            ...
            if (is.null(out[[k]])) {
                ...
                if (!is.null(out[[k]]$processed$classification)) {
                    try({
                        tempcols <- setdiff(names(out[[k]]$
                                                      processed$classification),
                                            unwanted_columns(type = "general"))
                        out[[k]]$processed$classification <-
                            out[[k]]$processed$classification[tempcols]
                        names(out[[k]]$processed$classification) <-
                            str_replace_all(names(out[[k]]$
                                                      processed$classification),
                                            "^classs", "class")
                        names(out[[k]]$processed$classification) <-
                            rename_variables(
