...
suppressWarnings( {
out = fromJSON(file=paste("http://bie.ala.org.au/ws/search/auto.json?q=",taxon,"&limit=",limit,sep="")) #download the data
})
tfun = function(x) { return(c( guid=ifelse(is.null(x$guid),NA,x$guid),
...
rankId=ifelse(is.null(x$rankId),NA,x$rankId),
