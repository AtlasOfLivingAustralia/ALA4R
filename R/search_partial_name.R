...
#' the taxon are known, use \code{\link{search_names}} instead.
#'
#' @references Associated ALA web service: \url{https://api.ala.org.au/#ws25}
#' @seealso \code{\link{search_names}} for searching known scientific or common
#' taxonomic names
#' @param taxon string: part of the scientific, common name of the taxa
#' @param geo_only logical: if TRUE, only results that have geospatial
#' occurrence records will be included
...
#' "simple"
#' @param index_type string: the index type to limit. Values include: TAXON
#' REGION COLLECTION INSTITUTION DATASET. By default, no index_type limit is
#' applied
#' @param limit numeric: the maximum number of matches returned (defaults to
#' the server-side value - currently 10)
#' @return A dataframe of results. The contents (column names) of the data
...
#' }
#' @export
search_partial_name <- function(taxon, geo_only = FALSE,
                                output_format = "simple", index_type, limit) {
    ...
    # replace spaces with + to force both terms in the search
    taxon <- gsub(" ", "+", taxon)

    this_query <- list(q = taxon)
    if (!missing(limit)) {
        # check limit is integer >0 and single value
...
"print.search_partial_name" <- function(x, ...) {
    ...
    if (identical(attr(x, "output_format"), "simple")) {
        cols <- intersect(c("name", "commonName", "matchedNames", "rankString",
                            "guid"), cols)
    }
